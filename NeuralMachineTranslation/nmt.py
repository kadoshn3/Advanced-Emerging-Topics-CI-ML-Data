...
# Encode sentence so model can understand text
def preprocess_sentence(w):
    ...
    w = w.strip()
    # Adds tokens so software knows where to start and end a sentence
    w = '<start> ' + w + ' <end>'
    return w
...
def create_df(path, num_examples):
    lines = io.open(path, encoding='UTF-8').read().strip().split('\n')

    word_pairs = [[preprocess_sentence(w) for w in l.split('\t')]  for l in lines[:num_examples]]
    
    # Convert to dataframe
    data_df = pd.DataFrame(word_pairs, columns=['en', 'sp', 'N/A'])
    
    # Only store english and spanish columns
    data_df = data_df.drop(columns='N/A')
...
def load_dataset(path, num_examples=None):
    ...
    data_df = create_df(path, num_examples)
    targ_lang, inp_lang = data_df['en'].values, data_df['sp'] 
